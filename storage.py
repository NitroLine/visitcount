...
class VisitInfo:
    ...
    def __str__(self):
        return f'{self.date} | {self.origin}{self.path} from {self.client_id}-{self.language}-{self.platform}-{self.browser}'
...
class RedisStorage:
    ...
    def get_all_clients(self, origin, date):
        return [i.decode() for i in self.redis.sscan(f'{date}:{origin}:clients')[1]]
    ...
    def get_origin_statistics_at_date(self, origin, date):
        ...
        total_referer_stats = {}
        for path in paths:
            referers = [i.decode() for i in self.redis.sscan(f'{date}:{origin}:referer:{path}')[1]]
            referer_stats = {}
            for ref in referers:
                referer_stats[ref] = self.redis.hget(f'{date}:{origin}:referers_count', ref)
